...
# Define a class to store process variables for image processing and UI
class AppState:
...
# Create pipeline and config objects for intel reslsense camera
pipeline = rs.pipeline()
config = rs.config()

# Enable depth and colors sensor feeds
...
# Get the active pipeline profiles
profile = pipeline.get_active_profile()

# Get sensor map dimensions from the depth stream
...
decimate = rs.decimation_filter()                                       # Decimation object (reduces point cloud size)
decimate.set_option(rs.option.filter_magnitude, 2 ** state.decimate)
colorizer = rs.colorizer()                                              # Colorizer object (applies colormap)
...
# Subscribe window for key events
...
def run(dt):
    ...
    # Color map assigned from depth data or rgb sensor
    if state.color:
